...
const Reminder = () => {
    const [data] = useState(reminderData)
    ...
    return (
        ...
        <div className={`pt-[6rem] pl-4 pr-4 font-poppinsFont bg-[#E6F1FD] h-[100vh]  pb-4 z-[-30] overflow-y-hidden`}>
            ...
            <div className='flex justify-between items-center flex-wrap mb-3 sm:mb-0'>
                <h1 className='text-xl font-bold tracking-wide mb-4 pt-4 '>Reminders</h1>
                <div onClick={() => nav('/lab/booking')}>
                    <Button value="Add Reminder" className='w-[8rem] h-[2.3rem] bg-[#1971F5] text-white rounded-[8px] md:text-sm text-xs' onClick={()=>setshowDetails(true)} />
                </div>
            </div>
            ...
            <div className='w-full overflow-x-scroll md:overflow-x-auto bg-white rounded-[16px]'>
                <table className='w-full'>
                    <thead>
                        <tr>
                            <th className='p-4 text-start border-b-2 border-[#E9EBED] tracking-wider text-sm  whitespace-nowrap'>Title</th>
                            <th className='p-4 text-start border-b-2 border-[#E9EBED] tracking-wider text-sm  whitespace-nowrap'>Date</th>
                            <th className='p-4 text-start border-b-2 border-[#E9EBED] tracking-wider text-sm  whitespace-nowrap'>Time</th>
                            ...
                            <tr key={index}>
                                ...
                                <td className='p-4 border-b-2 border-[#E9EBED] text-sm whitespace-nowrap'>
                                    <div className='flex gap-4'>
                                        ...
                                        <div className='w-[1.5rem] h-[1.5rem] rounded-full flex justify-center items-center bg-[#E6F1FD] cursor-pointer' onClick={() => setshowDetails(true)}>
                                            <AiOutlineDownload className='text-[#1971F5]' />
                                        </div>
                                        <div className='w-[1.5rem] h-[1.5rem] rounded-full flex justify-center items-center bg-[#E6F1FD] cursor-pointer' onClick={() => setshowDetails(true)}>
                                            <FaTrash className='text-[#1971F5] text-xs' />
                                        </div>
                ...
                showDetails ?
                    <div className='fixed top-0 right-0 overflow-y-scroll h-fit w-screen'>
                        ...
                        <div className='w-[20rem] h-full overflow-y-scroll bg-white fixed top-0 right-0'>
                            <ReminderPopup setShowPopup={setshowDetails} />
                        </div>
                    </div>
                    : ""
            }
